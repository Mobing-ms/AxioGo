...
export const ActionsView = () => {
  ...
  useEffect(() => {
    const elements = document.querySelectorAll('.action-scroll-reveal');
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-axio-bg">
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-20">
        ...
        <section
          className="action-scroll-reveal action-reveal mb-14"
        >
        ...
        <section
          className="action-scroll-reveal action-reveal mb-12"
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {actionsList.map((act, index) => (
            <article
              key={act.id}
              className="action-scroll-reveal action-reveal group relative overflow-hidden rounded-2xl bg-axio-panel/55 backdrop-blur-xl p-6 sm:p-7 transition-all duration-500 hover:-translate-y-1"
              style={{
            ...
            >
              ...
              <div className="p-4 rounded-xl bg-axio-bg/45 space-y-2 text-[10px] mb-6">
                <div>
                  <span className="text-axio-muted">Requested By:</span>{' '}
                  <span className="text-white">{act.requestedBy}</span>
                </div>

                <div>
                  <span className="text-axio-muted">Target System:</span>{' '}
                  <span className="text-axio-red">{act.targetSystem}</span>
                </div>

                <div>
                  <span className="text-axio-muted">Impact:</span>{' '}
                  <span className="text-axio-text-sub">{act.impact}</span>
                </div>
              </div>
            ...
            <div className="relative p-7 sm:p-8">
              ...
              <div className="space-y-5 text-xs font-sans mb-7">
                ...
                <p className="text-axio-text-secondary leading-relaxed">
                  This execution will commit Databricks production resources
                  and mutate operational shop schedules. Explicit Admin
                  approval is required by enterprise policy.
                </p>
              ...
              <div className="flex flex-col-reverse sm:flex-row items-center justify-end gap-3 pt-5">
                <button
                  onClick={() =>
                    handleRejectHighRisk(selectedActionModal.id)
                  }
                  disabled={isProcessingAction}
                ...
                {permissions.canApproveHighRiskActions ? (
                  <button
                    onClick={() =>
                      handleApproveHighRisk(selectedActionModal.id)
                    }
                    disabled={isProcessingAction}
